...
# 得到初始化模型
mobilenetModel = MobileNetV3(16, 8, 1, optimizer, loss_function, metrics, classes)
...
# 得到模型
train_model = mobilenetModel.create_model()

# 模型训练，得到训练过程中的精度和loss曲线
H = train_model.fit(X_train, Y_train, epochs=30, batch_size=8, validation_data=(X_val, Y_val))
...
result.to_csv('mobilenetv3_submit.csv', index=None)
...
plt.title("Training Loss and Accuracy for MobileNetV3 model.")
plt.xlabel("Epoch #")
plt.ylabel("Loss/Accuracy")
plt.legend()
plt.savefig("MobileNetV3_train_monitor.png")
plt.close()
